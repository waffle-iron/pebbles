...
def proxy_add_route(route_key, target, no_rewrite_rules=False):
    ...
    template = Template(
        ...
        location /${route_key}/ {
          ...
          proxy_pass ${target};
          ${no_rw}proxy_redirect ${target} $$scheme://$$host:${proxy_public_https_port}/${route_key};
    ...
    with open(path, 'w') as f:
        f.write(
            template.substitute(
                route_key=route_key,
                target=target,
                proxy_public_https_port=get_config()['PROXY_PUBLIC_HTTPS_PORT'],
                no_rw=no_rw
            )
        )
...
def refresh_nginx_config():
    config = [
        'server {',
        '   listen %s;' % get_config()['PROXY_INTERNAL_HTTPS_PORT'],
        '   ssl on;',
